...
q_amp = np.array([lead[peak] if isinstance(peak, int) else np.nan for peak in q_peaks])
r_amp = np.array([lead[peak] if isinstance(peak, int) else np.nan for peak in r_peaks])
s_amp = np.array([lead[peak] if isinstance(peak, int) else np.nan for peak in s_peaks])
p_amp = np.array([lead[peak] if isinstance(peak, int) else np.nan for peak in p_peaks])
t_amp = np.array([lead[peak] if isinstance(peak, int) else np.nan for peak in t_peaks])
...
q_int = get_intervals(q_peaks, 'rr')
pct_q = get_pct_change(q_int)
print(q_int)
print(pct_q)
print(np.abs(np.array(pct_q) * 100))
